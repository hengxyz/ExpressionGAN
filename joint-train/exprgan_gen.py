...
class ExprGAN(object):
    def __init__(self,
                 ...
                 q_coeff=1,
                 fm_coeff=1,
                 vgg_face='/data/zming/models/GAN/ExprGAN/vgg-face.mat',
        ...
        self.vgg_weights = loadmat(vgg_face)
        ...
        self.real_conv1_2, self.real_conv2_2, self.real_conv3_2, self.real_conv4_2, self.real_conv5_2 = self.face_embedding(
            self.input_image)
        self.fake_conv1_2, self.fake_conv2_2, self.fake_conv3_2, self.fake_conv4_2, self.fake_conv5_2 = self.face_embedding(
            self.G)
    ...
    def train(self,
        ...
        for epoch in range(num_epochs):
            ...
            for ind_batch in range(num_batches):
                ...
                print("\tBatch elapse time: %.04f s/%d images" % (elapse, self.size_batch))
    ...
    def encoder(self, image, reuse_variables=False):
        ...
        ## The conv stride is 2, so every time the feature map size will be /2 until 1 as possible, so the limit of the
        ## number of the layers of the encoder is given by:
    ...
    def generator(self, z, z_noise, y, reuse_variables=False, enable_tile_label=True, tile_ratio=1.0):
        if reuse_variables:
            tf.get_variable_scope().reuse_variables()
        ## To generate the z to a image, every time upsampling the z (*2) from size_mini_map (4) to the size_image (128)
        num_layers = int(np.log2(self.size_image)) - int(self.size_kernel / 2)
        ...
        for i in range(num_layers):
            name = 'G_deconv' + str(i)
            current = tf.image.resize_nearest_neighbor(current,
                                                       [size_mini_map * 2 ** (i + 1), size_mini_map * 2 ** (i + 1)])
    ...
    def gen_im(self, images, names, test_dir, dataset, G_num, batch_z_prior):

        size_sample = images.shape[0]
        ## 6 different expressions
        labels = np.arange(self.y_dim)
        # labels = (3* np.ones(self.y_dim)).astype(int)
        ...
        if G.shape[0] != query_images.shape[0]:
	    print('Generating problems! Generating %d images / %d source images'%(G.shape[0], query_images.shape[0]))
        save_files = []
        cnt = 0
        for i in range(self.y_dim):
            for j, file in enumerate(names):
                filename = str.split(file, '.')
                suffix = filename[1]
                filename = str.split(filename[0], '/')
                if dataset == 'FER2013':
                    save_file = filename[-1] + '_%d' % i + '_%d.' % (G_num+cnt) + suffix
                if dataset == 'OULU' or dataset == 'CK':
                    save_file = filename[-3] +'_' + filename[-2] +'_' + filename[-1] + '_%d' % i + '_%d.' % (G_num+cnt) + suffix
                save_files.append(save_file)
                cnt += 1
       
        save_num = save_single_images(batch_images=G, save_path=test_dir, save_files=save_files, image_value_range=self.image_value_range)
        return G.shape[0], save_num     
    ...
    def custom_test(self, testing_samples_dir, random_seed):
        ...
        if testing_samples_dir:
            #FER2013
            file_names = glob(testing_samples_dir, '*.png')
            ## Oulu or CK+
    ...
    def gen_images(self, testing_samples_dir, random_seed):
        if not self.load_checkpoint():
            print("\tFAILED >_<!")
            exit(0)
        else:
        ...
        save_test_dir = os.path.join(self.save_dir, 'test')
        ...
        if testing_samples_dir:
            ## FER2013
            #file_names = glob(os.path.join(testing_samples_dir, '*.png'))
            file_names = os.listdir(testing_samples_dir)
            ## Oulu or CK+
            if os.path.isdir(os.path.join(testing_samples_dir, file_names[0])):
                dataset = 'OULU'
                identies = file_names
                file_names = []
                for id in identies:
                    exprs = os.listdir(os.path.join(testing_samples_dir,id))
                    for expr in exprs:
                        files = os.listdir(os.path.join(testing_samples_dir, id, expr))
                        for file in files:
                            file_names.append(os.path.join(testing_samples_dir, id, expr, file))
            else:
                dataset = 'FER2013'
                for i, file_name in enumerate(file_names):
                    file_names[i] = os.path.join(testing_samples_dir, file_name)
        else:
            np.random.seed(random_seed)
            file_names = self.data_X[num_samples:2 * num_samples]
        print('============== %d, %d '%((self.generate_num//self.y_dim), len(file_names)))
        if( (self.generate_num//self.y_dim) > len(file_names)):
            file_names = np.random.choice(file_names, self.generate_num // self.y_dim, replace=True)
        else:
            file_names = np.random.choice(file_names, self.generate_num//self.y_dim, replace=False)
        ##file_names = list(set(file_names))
        print('Selecting %d images as source to generate new images....'%len(file_names))
        if len(file_names) < num_samples:
            print 'The number of testing images must be larger than %d' % num_samples
            exit(0)

        num_batches = len(file_names) // num_samples
        G_num = 0
        Save_num = 0
        sample_z_prior = np.random.uniform(
                    self.image_value_range[0],
                    self.image_value_range[-1],
                    [self.size_batch//self.y_dim, self.num_z_channels]
                ).astype(np.float32)
        batch_z_prior = np.tile(sample_z_prior, [self.y_dim, 1])  

        for i in range(num_batches):
            print('batch %d...'%i)
            print('%d-%d images generating for %d expressions'%(i * num_samples*self.y_dim, (i + 1) * num_samples*self.y_dim, self.y_dim))
            sample_files = file_names[i * num_samples:(i + 1) * num_samples]
            #print sample_files
            sample = [load_image(
                image_path=sample_file,
                image_size=self.size_image,
            ...
            g_num, save_num  = self.gen_im(images, sample_files, save_test_dir, dataset, G_num, batch_z_prior)
            G_num += g_num
            Save_num += save_num
            print('>>>>>>>>> Generating %d images, saving %d images'%(G_num, Save_num))
    ...
    def y_to_rb_label_intensity(self, label, i):
        number = np.argmax(label)
        # one_hot = np.random.uniform(-1, 1, self.rb_dim)
        one_hot = np.tile(i / self.y_dim, self.rb_dim)
        rb = np.tile(-1 * np.abs(one_hot), self.y_dim)
        rb[number * self.rb_dim:(number + 1) * self.rb_dim] = np.abs(one_hot)
        return rb
